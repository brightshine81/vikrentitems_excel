...
class VikrentitemsController extends JControllerLegacy {
	
	function display($cachable = false, $urlparams = array()) {
		$view = VikRequest::getVar('view', '');
		if ($view == 'itemslist') {
			VikRequest::setVar('view', 'itemslist');
		} elseif ($view == 'itemdetails') {
			VikRequest::setVar('view', 'itemdetails');
		} elseif ($view == 'loginregister') {
			VikRequest::setVar('view', 'loginregister');
		} elseif ($view == 'locationsmap') {
			VikRequest::setVar('view', 'locationsmap');
		} elseif ($view == 'locationslist') {
			VikRequest::setVar('view', 'locationslist');
		} elseif ($view == 'userorders') {
			VikRequest::setVar('view', 'userorders');
		} else {
	...
	function emptycart() {
		...
		if ($psearch == 1) {
			$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&view=vikrentitems" . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
		} elseif ($pdetails == 1 && $pelemid > 0) {
			$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&view=itemdetails&elemid=" . $pelemid . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
		} else {
			$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&task=oconfirm&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
		}
	}
		...
		if (!VikRentItems::userIsLogged()) {
			if (!empty($pname) && !empty($plname) && !empty($pusername) && VikRentItems::validEmail($pemail) && $ppassword == $pconfpassword) {
				...
				if ($newuserid!=false && strlen($newuserid)) {
					...
					//
					if (!empty($ppriceid)) {
						$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&task=oconfirm&'.$qstring, false));
					} else {
						//$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&view=userorders', false));
						$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&task=oconfirm&'.$qstring, false));
					}
				} else {
					//error while saving new user
					VikError::raiseWarning('', JText::_('VRIREGERRSAVING'));
					$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&view=loginregister&'.$qstring, false));
				}
			} else {
				//invalid data
				VikError::raiseWarning('', JText::_('VRIREGERRINSDATA'));
				$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&view=loginregister&'.$qstring, false));
			}
		} else {
	...
	function saveorder() {
		...
		if ($validtoken) {
			...
			if (count($vricart) == 0) {
				...
				$session->set('vriCart', '');
				$session->set('vriDeliveryCart', '');
				VikError::raiseWarning('', JText::_('VRICARTISEMPTY'));
				$mainframe = JFactory::getApplication();
				$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&task=oconfirm&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
				exit;
			}
			...
			if (@is_array($cfields)) {
				...
				foreach ($cfields as $cf) {
					...
					if ($cf['type'] != 'separator' && $cf['type'] != 'country' && ( $cf['type'] != 'checkbox' || ($cf['type'] == 'checkbox' && intval($cf['required']) != 1) ) ) {
						$arrcustdata[JText::_($cf['name'])] = VikRequest::getString('vrif' . $cf['id'], '', 'request');
					...
					foreach ($vricart as $iditem => $itemarrparent) {
						foreach ($itemarrparent as $k => $itemarr) {
							...
							if (is_array($tar)) {
								...
								$session->set('vriCart', $vrisessioncart);
								VikError::raiseWarning('', JText::_('VRIERRITEMFARENOTFOUND'));
								$mainframe = JFactory::getApplication();
								$mainframe->redirect(JRoute::_("index.php?option=com_vikrentitems&task=oconfirm&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
								break;
							}
							...
							if ($validlocations == true) {
								...
								if ($itemsnotlocked === true) {
									if ($itemsnotbooked === true) {
										...
										$viklink = JURI::root() . "index.php?option=com_vikrentitems&task=vieworder&sid=" . $sid . "&ts=" . $nowts . (!empty($pitemid) ? "&Itemid=" . $pitemid : "");
										...
										if (VikRentItems::areTherePayments()) {
											...
											if (is_array($payment)) {
												...
												if (intval($payment['setconfirmed']) == 1) {
													...
													VikRentItems::sendAdminMail($admail.';;'.$useremail, JText::_('VRORDNOL'), $ftitle, $neworderid, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $payment['name'], $strcouponeff, $totdelivery);
													VikRentItems::sendCustMail($useremail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
													if (count($extra_emails)) {
														foreach ($extra_emails as $extraemail) {
															VikRentItems::sendCustMail($extraemail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
														}
													}
													$mainframe = JFactory::getApplication();
													$mainframe->redirect("index.php?option=com_vikrentitems&task=vieworder&sid=" . $sid . "&ts=" . $nowts . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""));
												} else {
													...
													VikRentItems::sendAdminMail($admail.';;'.$useremail, JText::_('VRORDNOL'), $ftitle, $neworderid, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, JText::_('VRINATTESA'), $ritplace, $consegnaplace, $maillocfee, $payment['name'], $strcouponeff, $totdelivery);
													VikRentItems::sendCustMail($useremail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRINATTESA'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
													if (count($extra_emails)) {
														foreach ($extra_emails as $extraemail) {
															VikRentItems::sendCustMail($extraemail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRINATTESA'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
														}
													}
													$mainframe = JFactory::getApplication();
													$mainframe->redirect("index.php?option=com_vikrentitems&task=vieworder&sid=" . $sid . "&ts=" . $nowts . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""));
												...
												$mainframe = JFactory::getApplication();
												$mainframe->redirect("index.php?option=com_vikrentitems&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . "&task=oconfirm");
											}
										} else {
											$realback = VikRentItems::getHoursItemAvail() * 3600;
											$realback += $prelease;
											...
											VikRentItems::sendAdminMail($admail.';;'.$useremail, JText::_('VRORDNOL'), $ftitle, $neworderid, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $strcouponeff, $totdelivery);
											VikRentItems::sendCustMail($useremail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
											if (count($extra_emails)) {
												foreach ($extra_emails as $extraemail) {
													VikRentItems::sendCustMail($extraemail, strip_tags($ftitle) . " " . JText::_('VRORDNOL'), $ftitle, $nowts, $custdata, $vricart, $ppickup, $prelease, $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $neworderid, $strcouponeff, $totdelivery);
								...
								showSelectVRI(JText::_('VRINVALIDLOCATIONS'));
							}
						} else {
							showSelectVRI(JText::_('VRINVALIDDATES'));
						}
					} else {
						showSelectVRI(JText::_('VRINCONGRTOT'));
					}
				} else {
					showSelectVRI(JText::_('VRINCONGRDATA'));
				}
			} else {
				showSelectVRI(JText::_('VRINSUFDATA'));
			}
		} else {
			showSelectVRI(JText::_('VRINVALIDTOKEN'));
		}
	}
		...
		if (!empty($sid) && !empty($ts)) {
			...
			if ($dbo->getNumRows() == 1) {
				...
				if ($order[0]['status'] == "confirmed") {
					...
					$itemavail = true;
					$q = "SELECT `oi`.*,`i`.`units` FROM `#__vikrentitems_ordersitems` AS `oi`, `#__vikrentitems_items` AS `i` WHERE `oi`.`idorder`=".(int)$order[0]['id']." AND `oi`.`iditem`=`i`.`id`;";
					$dbo->setQuery($q);
					$dbo->execute();
					if ($dbo->getNumRows() > 0) {
						$orderitems = $dbo->loadAssocList();
						foreach ($orderitems as $oi) {
							$itemavail = VikRentItems::itemBookable($oi['iditem'], $oi['units'], $order[0]['ritiro'], $order[0]['consegna'], $oi['itemquant']);
						}
					} else {
						$itemavail = false;
					}
					...
					if ($itemavail === true) {
						...
						$q = "UPDATE `#__vikrentitems_orders` SET `status`='cancelled' WHERE `id`=".(int)$order[0]['id'].";";
						$dbo->setQuery($q);
						$dbo->execute();
						$q = "DELETE FROM `#__vikrentitems_tmplock` WHERE `idorder`=" . (int)$order[0]['id'] . ";";
						$dbo->setQuery($q);
						$dbo->execute();
						$q = "SELECT * FROM `#__vikrentitems_ordersbusy` WHERE `idorder`=".(int)$order[0]['id'].";";
						$dbo->setQuery($q);
						$dbo->execute();
						if ($dbo->getNumRows() > 0) {
							$allbusy = $dbo->loadAssocList();
							foreach ($allbusy as $b) {
								$q = "DELETE FROM `#__vikrentitems_busy` WHERE `id`=" . (int)$b['idbusy'] . " LIMIT 1;";
								$dbo->setQuery($q);
								$dbo->execute();
							}
						}
						$q = "DELETE FROM `#__vikrentitems_ordersitems` WHERE `idorder`=".(int)$order[0]['id'].";";
						$dbo->setQuery($q);
						$dbo->execute();
						$q = "DELETE FROM `#__vikrentitems_ordersbusy` WHERE `idorder`=".(int)$order[0]['id'].";";
						$dbo->setQuery($q);
						$dbo->execute();
						if ($today_time > $order[0]['ritiro']) {
							VikError::raiseWarning('', JText::_('VRIBOOKNOLONGERPAYABLE'));
						} else {
							VikError::raiseWarning('', JText::_('VRERRREPSEARCH'));
						}
						//Show the cancelled order details page in any case
						VikRequest::setVar('view', 'standbyorder');
						parent::display();
					}
				}
			} else {
				showSelectVRI(JText::_('VRORDERNOTFOUND'));
			}
		} else {
			showSelectVRI(JText::_('VRINSUFDATA'));
		}
	...
	function cancelrequest() {
		...
		if (!empty($psid) && !empty($pidorder)) {
			$q = sprintf("SELECT * FROM `#__vikrentitems_orders` WHERE `id`='%d' AND `sid`=".$dbo->quote($psid).";", $pidorder);
			$dbo->setQuery($q);
			$dbo->execute();
			if ($dbo->getNumRows() == 1) {
				$order = $dbo->loadAssocList();
				...
				if (!empty($pemail) && !empty($preason)) {
					...
					$subject = JText::_('VRICANCREQUESTEMAILSUBJ');
					$subject = '=?UTF-8?B?' . base64_encode($subject) . '?=';
					$msg = JText::sprintf('VRICANCREQUESTEMAILHEAD', $order[0]['id'], JURI::root().'index.php?option=com_vikrentitems&task=vieworder&sid='.$order[0]['sid'].'&ts='.$order[0]['ts'])."\n\n".$preason;
					...
					$mailer->Send();
					$mainframe->enqueueMessage(JText::_('VRICANCREQUESTMAILSENT'));
					$mainframe->redirect("index.php?option=com_vikrentitems&task=vieworder&sid=".$order[0]['sid']."&ts=".$order[0]['ts']."&Itemid=".VikRequest::getString('Itemid', '', 'request'));
				} else {
					$mainframe->redirect("index.php?option=com_vikrentitems&task=vieworder&sid=".$order[0]['sid']."&ts=".$order[0]['ts']);
				}
			} else {
		...
		if (strlen($psid) && strlen($pts)) {
			...
			if ($dbo->getNumRows() > 0) {
				...
				if ($rows[0]['status']!='confirmed') {
					...
					$obj = new vikRentItemsPayment($rows[0], json_decode($payment['params'], true));
					...
					if ($array_result['verified'] == 1) {
						...
						$ftitle = VikRentItems::getFrontTitle($vri_tn);
						$nowts = time();
						$viklink = JURI::root() . "index.php?option=com_vikrentitems&task=vieworder&sid=" . $psid . "&ts=" . $pts;
						$checkhourscharges = 0;
						$hoursdiff = 0;
						...
						if (array_key_exists('tot_paid', $array_result)) {
							...
							if ($shouldpay != $totreceived) {
								...
								$mailer->setSubject(JText::_('VRITOTPAYMENTINVALID'));
								$mailer->setBody(JText::sprintf('VRITOTPAYMENTINVALIDTXT', $rows[0]['id'], $totreceived." (".$array_result['tot_paid'].")", $shouldpay));
								$mailer->isHTML(false);
								$mailer->Encoding = 'base64';
								$mailer->Send();
							}
						...
						$dbo->execute();
						//
						VikRentItems::sendAdminMail($admail.';;'.$rows[0]['custmail'], JText::_('VRRENTALORD'), $ftitle, $rows[0]['id'], $nowts, $rows[0]['custdata'], $vricart, $rows[0]['ritiro'], $rows[0]['consegna'], $isdue, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $payment['name'], $rows[0]['coupon'], $totdelivery);
						VikRentItems::sendCustMail($rows[0]['custmail'], strip_tags($ftitle) . " " . JText::_('VRRENTALORD'), $ftitle, $nowts, $rows[0]['custdata'], $vricart, $rows[0]['ritiro'], $rows[0]['consegna'], $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $rows[0]['id'], $rows[0]['coupon'], $totdelivery);
						if (count($extra_emails)) {
							foreach ($extra_emails as $extraemail) {
								VikRentItems::sendCustMail($extraemail, strip_tags($ftitle) . " " . JText::_('VRRENTALORD'), $ftitle, $nowts, $rows[0]['custdata'], $vricart, $rows[0]['ritiro'], $rows[0]['consegna'], $isdue, $viklink, JText::_('VRIOMPLETED'), $ritplace, $consegnaplace, $maillocfee, $rows[0]['id'], $rows[0]['coupon'], $totdelivery);
							}
						}
	...
	function ajaxlocopentime() {
		...
		$dbo = JFactory::getDBO();
		$ret = array();
		$q = "SELECT `opentime` FROM `#__vikrentitems_places` WHERE `id`='".$pidloc."';";
		$dbo->setQuery($q);
		$dbo->execute();
