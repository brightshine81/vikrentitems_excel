...
class VikRentItemsCustomersPin {
	...
	*/
	private function setPinCookie($pin) {
		...
		if (!empty($pin)) {
			...
			$cookie = JFactory::getApplication()->input->cookie;
			$cookie->set( 'vrcPinData', $pin_cookie, (time() + (86400 * 365)), '/' );
		}
	...
	*/
	public function pluginCustomerSync($customer_id, $mode) {
		...
		$q = "SELECT * FROM `#__vikrentitems_customers` WHERE `id`=".(int)$customer_id." LIMIT 1;";
		$this->dbo->setQuery($q);
		$this->dbo->execute();
		if ($this->dbo->getNumRows() > 0) {
			$customer = $this->dbo->loadAssoc();
			//Launch plugin
			$options = array(
				'alias' 	=> 'com_vikrentitems',
				'version' 	=> E4J_SOFTWARE_VERSION,
				'admin' 	=> (method_exists($mainframe, 'isAdmin') ? $mainframe->isAdmin() : true),
				'call' 		=> __FUNCTION__
			);
			...
			if (class_exists('JEventDispatcher')) {
				...
				if ($mode == 'insert') {
					...
					$dispatcher->trigger('onCustomerInsert', array(&$customer, &$options));
				} elseif ($mode == 'update') {
					// trigger plugin -> customer update
					$dispatcher->trigger('onCustomerUpdate', array(&$customer, &$options));
				} else {
					// trigger plugin -> customer delete
					$dispatcher->trigger('onCustomerDelete', array(&$customer, &$options));
				}
			} else {
				if (method_exists($mainframe, 'triggerEvent')) {
					if ($mode == 'insert') {
						// trigger plugin -> customer creation
						$mainframe->triggerEvent('onCustomerInsert', array(&$customer, &$options));
					} elseif ($mode == 'update') {
						// trigger plugin -> customer update
						$mainframe->triggerEvent('onCustomerUpdate', array(&$customer, &$options));
					} else {
						// trigger plugin -> customer delete
						$mainframe->triggerEvent('onCustomerDelete', array(&$customer, &$options));
					}
				}
			}
			//
