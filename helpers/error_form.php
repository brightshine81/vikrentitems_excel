...
function vriGlobalClosingDays(date) {
	...
	if (is_array($vrisessioncart) && count($vrisessioncart) > 0) {
		...
		$selform .= "<div class=\"vrisearchemptycartdiv\"><a href=\"".JRoute::_('index.php?option=com_vikrentitems&task=emptycart&search=1')."\">".JText::_('VRIEMPTYCARTCHANGEDATES')."</a></div>\n";
	}
...
jQuery(function(){
	...
	if (count($coordsplaces) > 0) {
		$selform = '<div class="vrilocationsbox"><div class="vrilocationsmapdiv"><a href="'.JRoute::_('index.php?option=com_vikrentitems&view=locationsmap&tmpl=component').'" class="vrimodal" target="_blank"><img src="'.VRI_SITE_URI.'resources/images/mapslocations-small.png"/><span>'.JText::_('VRILOCATIONSMAP').'</span></a></div></div>'.$selform;
		?>
	...
	if (is_array($vrisessioncart) && count($vrisessioncart) > 0) {
		...
		<div class="vrisearchgosummarydiv">
			<a href="<?php echo JRoute::_('index.php?option=com_vikrentitems&task=oconfirm&place='.$vrisesspickuploc.'&returnplace='.$vrisessdropoffloc.'&days='.$vrisessdays.'&pickup='.$vrisesspickup.'&release='.$vrisessdropoff); ?>"><?php echo JText::_('VRIGOTOSUMMARY'); ?></a>
		</div>
		<?php
	}
