...
class VikrentitemsViewShowprc extends JViewLegacy {
			...
			if ($dbo->getNumRows() > 0) {
				...
				if ($dbo->getNumRows() == 1) {
					...
					if (intval($item[0]['avail']) == 1 && $quantity_allowed === true) {
						...
						showSelectVRI(JText::_('VRIARNOTAV'));
					}
				} else {
					showSelectVRI(JText::_('VRIARNOTFND'));
				}
			} else {
				showSelectVRI(JText::_('VRNOTARFNDSELO'));
			}
		} else {
			showSelectVRI(JText::sprintf('VRIALLUNITSNOTRIT', $pitemquant) . " " . date($df . ' H:i', $ppickup) . " " . JText::_('VRIARNOTCONSTO') . " " . date($df . ' H:i', $prelease));
		}
	}
}
