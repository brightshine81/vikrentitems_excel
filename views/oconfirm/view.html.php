...
class VikrentitemsViewOconfirm extends JViewLegacy {
			...
			if (is_array($coupon)) {
				...
				if ($coupondateok == true) {
					...
					if ($couponitemok !== true) {
						VikError::raiseWarning('', JText::_('VRICOUPONINVITEM'));
						$coupon = "";
					}
				} else {
					VikError::raiseWarning('', JText::_('VRICOUPONINVDATES'));
					$coupon = "";
				}
			} else {
				VikError::raiseWarning('', JText::_('VRICOUPONNOTFOUND'));
				$coupon = "";
			}
		}
		$this->coupon = $coupon;
							...
							if (VikRentItems::dayValidTs($pdays, $ppickup, $prelease)) {
								...
								if (intval($pdelivery) == 1 && (int)$pdeliverysessionval > 0 && count($vrideliverycart) > 0) {
									if (array_key_exists($pdeliverysessionval, $vrideliverycart)) {
										...
										VikError::raiseWarning('', JText::_('VRIDELIVERYSESSERR'));
									}
								}
								...
								$app->redirect(JRoute::_("index.php?option=com_vikrentitems&task=oconfirm&place=" . $pplace . "&returnplace=" . $preturnplace . "&days=" . $pdays . "&pickup=" . $ppickup . "&release=" . $prelease . (!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
								exit;
								//
								...
								showSelectVRI(JText::_('VRERRCALCTAR'));
							}
						} else {
							showSelectVRI(JText::_('VRTARNOTFOUND'));
						}
					} else {
						showSelectVRI(JText::_('VRNOTARSELECTED'));
					}
				} else {
					showSelectVRI(JText::_('VRIARNOTCONS') . " " . date($df . ' H:i', $ppickup) . " " . JText::_('VRIARNOTCONSTO') . " " . date($df . ' H:i', $prelease));
				}
			} else {
				showSelectVRI(JText::_('VRIARNOTFND'));
			}
		}
	}
}
