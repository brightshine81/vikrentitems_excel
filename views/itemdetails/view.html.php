...
class VikrentitemsViewItemdetails extends JViewLegacy {
	function display($tpl = null) {
		...
		if ($dbo->getNumRows() == 1) {
			...
			$actnow=time();
			$q="SELECT * FROM `#__vikrentitems_busy` WHERE `iditem`='".$item[0]['id']."' AND (`ritiro`>=".$actnow." OR `consegna`>=".$actnow.");";
			$dbo->setQuery($q);
			$dbo->execute();
			if ($dbo->getNumRows() > 0) {
			...
			$mainframe->redirect("index.php?option=com_vikrentitems&view=itemslist");
		}
	}
}
