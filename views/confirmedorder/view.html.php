...
class VikrentitemsViewConfirmedorder extends JViewLegacy {
	function display($tpl = null) {
		...
		$q="SELECT `oi`.`iditem`,`oi`.`idtar`,`oi`.`optionals`,`oi`.`itemquant`,`oi`.`deliveryaddr`,`oi`.`deliverydist`,`oi`.`cust_cost`,`oi`.`cust_idiva`,`oi`.`extracosts`,`i`.`name`,`i`.`img`,`i`.`idcarat`,`i`.`info`,`i`.`moreimgs` FROM `#__vikrentitems_ordersitems` AS `oi`,`#__vikrentitems_items` AS `i` WHERE `oi`.`idorder`='".$order[0]['id']."' AND `oi`.`iditem`=`i`.`id` ORDER BY `oi`.`id` ASC;";
		$dbo->setQuery($q);
		$dbo->execute();
		if ($dbo->getNumRows() > 0) {
		...
		if ($theme != 'default') {
			$thdir = VRI_SITE_PATH.DS.'themes'.DS.$theme.DS.'confirmedorder';
			if (is_dir($thdir)) {
