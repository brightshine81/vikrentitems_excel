...
class VikrentitemsViewUserorders extends JViewLegacy {
	function display($tpl = null) {
		...
		if (!empty($pconfirmnumber)) {
			...
			if ($dbo->getNumRows() > 0) {
				...
				$mainframe->redirect(JRoute::_('index.php?option=com_vikrentitems&task=vieworder&sid='.$odata[0]['sid'].'&ts='.$odata[0]['ts'].(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false));
				exit;
			} else {
				if ($cpin->pinExists($pconfirmnumber)) {
					$cpin->setNewPin($pconfirmnumber);
				} else {
					VikError::raiseWarning('', JText::_('VRINVALIDCONFIRMNUMBER'));
				}
			}
		}
