...
function initialize() {
	...
	geocoder.geocode( { 'address': baseaddress}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			...
			jQuery("#vrideliverymapesit").text("<?php echo addslashes(JText::_('VRIDELMAPERR1')); ?>"+status);
		}
	});
}
...
?>
	...
	?>
		...
		var jqxhr = jQuery.ajax({
			...
			deliveryobj = jQuery.parseJSON(resp);
			deliverycost = deliverycost.toFixed(2);
			var esitstr = "<p class='vrimapdelivdistp'><?php echo addslashes(JText::_('VRIDELMAPDISTANCE')); ?> "+unitdist+" "+calcunit+"</p>";
			esitstr += "<p class='vrimapdelivcostp'><?php echo addslashes(JText::_('VRIDELMAPCOST')); ?> "+currency+" "+deliverycost+"</p>";
			...
			jQuery("#vrideliverymapesit").addClass("vrimaperr");
			jQuery("#vrideliverymapesit").text('<?php echo addslashes(JText::_('VRIDELMAPERR5')); ?>');
			jQuery("#vrideliverymapcontinue").hide();
			vriPopulateParent(deliveryaddr, '', '', '');
		});
	} else {
		var disterrstr = '<?php echo addslashes(JText::_('VRIDELMAPERR4')); ?>';
	...
	directionsService.route(request, function(response, status) {
		if (status == google.maps.DirectionsStatus.OK) {
				...
				if (status == google.maps.GeocoderStatus.OK) {
					...
					geocoder.geocode( { 'address': end}, function(results, status) {
						if (status == google.maps.GeocoderStatus.OK) {
							...
							jQuery("#vrideliverymaploading").fadeOut();
							jQuery("#vrideliverymapesit").addClass("vrimaperr");
							jQuery("#vrideliverymapesit").text('<?php echo addslashes(JText::_('VRIDELMAPERR2')); ?>(' + status + ')');
						}
					});
				}
			});
			...
			if (status == google.maps.GeocoderStatus.NOT_FOUND || status == google.maps.GeocoderStatus.ZERO_RESULTS) {
				jQuery("#vrideliverymapesit").text('<?php echo addslashes(JText::_('VRIDELMAPERR3')); ?>(' + status + ')');
			} else {
				jQuery("#vrideliverymapesit").text('<?php echo addslashes(JText::_('VRIDELMAPERR2')); ?>(' + status + ')');
			}
			jQuery("#vrideliverymaploading").fadeOut();
...
?>
	<div class="vrideliveryinsaddr">
		<label for="deliveryTo"><?php echo JText::_('VRIDELIVERYADDRESSENTER'); ?></label>
		<input type="text" name="deliveryTo" id="deliveryTo" value="<?php echo $pdelto; ?>" size="30"/>
		<span id="vricalculate"><?php echo JText::_('VRIDELIVERYVALIDATEADDRESS'); ?></span>
	</div>
<?php
if (!empty($mapnotes)) {
	?>
...
?>
	...
	<div id="vrideliverymapcontinue">
		<button type="button" class="btn booknow" onclick="vriCloseDeliveryMap();"><i class="fas fa-check-circle"></i> <?php echo JText::_('VRIDELIVERYCONTINUEBTN'); ?></button>
	</div>
</div>
