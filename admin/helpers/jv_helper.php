...
if (!class_exists('VriApplication')) {
	class VriApplication {
		...
		{
			if (file_exists(VRI_ADMIN_PATH . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . $this->cms.'_'.$this->jv.'.css')) {
				JFactory::getDocument()->addStyleSheet(VRI_ADMIN_URI . 'helpers/' . $this->cms.'_'.$this->jv.'.css');
			}
		}
		...
		*/
		...
		<fieldset id="'.$name.'">
			...
			<span class="switcher-labels">
				<span class="switcher-label-'.$no_value.($cur_value === $no_value ? ' active' : '').'">'.JText::_('VRNO').'</span>
				<span class="switcher-label-'.$yes_value.($cur_value === $yes_value ? ' active' : '').'">'.JText::_('VRYES').'</span>
			</span>
		</fieldset>
	</div>';
			}
		...
		*/
		...
		public function getJmodalScript ($suffix = '', $hide_js = '', $show_js = '')
		{
			...
			if (!empty($hide_js)) {
				$hide_ev = "\n\t\t\t";
				$hide_ev .= "jQuery('.vri-jmodal').on('$hide_evname', function() {
				".$hide_js."
			});";
				$hide_ev .= "\n";
			}
		...
		function vriOpenJModal$suffix(id, modal_url, new_title) {
			if (modal_url) {
				...
				setTimeout(vriFocusModal.bind(id), 500);
			}
			if (new_title) {
		...
		jQuery(document).ready(function() {
			jQuery('.vri-jmodal').on('$show_evname', function() {
				$onshow_js
				$show_js
			});$hide_ev
		});
		...
		public function getJmodalHtml ($id, $title, $body = '', $style = '')
		{
			if ($this->jv < 4) {
				...
				return "\t\t".'<div class="modal vri-jmodal hide fade" id="jmodal-'.$id.'" style="'.$style.'">
				...
				return "\t\t".'<div class="joomla-modal vri-jmodal modal fade" id="jmodal-'.$id.'" role="dialog" tabindex="-1" style="'.$style.'">
		 ...
		 * @since 	1.12
		 */
		public function attachPopover($selector = '.vriPopover', $options = array())
		{
			if (defined('JVERSION') && version_compare(JVERSION, '2.6.0') < 0) {
				// 2.5
		 ...
		 * @param 	array 	$options    An array of options for the popover
		 *
		 * @see 	VriApplication::attachPopover() for further details about options keys.
		 *
		 * @since 	1.12
		 */
		public function createPopover($options = array())
		{
			...
			$options['icon_class'] = isset($options['icon_class']) ? $options['icon_class'] : 'vriicn-question';

			// attach an empty array option so that the data will be recovered 
			// directly from the tag during the runtime
			$this->attachPopover(".vri-quest-popover", array());

			if (defined('JVERSION') && version_compare(JVERSION, '2.6.0') < 0) {
				// 2.5
				return '<i class="'.$options['icon_class'].' vri-quest-popover" title="'.$options['content'].'"></i>';
			} else {
				// 3.x
				$attr = '';
				...
				return '<i class="'.$options['icon_class'].' vri-quest-popover" '.$attr.'></i>';
			}
		}
