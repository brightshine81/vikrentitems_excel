...
class vikRentItemsPayment {
	...
	static function getAdminParameters () {
		return array(
				...
				'tls12' => array('type' => 'select', 'label' => 'Force TLS 1.2://if ON, the connection to PayPal will be established only through the TLS 1.2 protocol', 'options' => array('OFF', 'ON'))
		);
	}
	...
	public function __construct ($order, $params = array()) {
		...
		$this->params['tls12'] = $this->params['tls12'] == 'ON' ? 1 : 0;
	}
	...
	public function showPayment () {
		...
		if($this->order_info['leave_deposit']) {
			$depositmess="<p><strong>".JText::_('VRLEAVEDEPOSIT')." ".$this->order_info['currency_symb']." ".VikRentItems::numberFormat($this->order_info['total_to_pay'])."</strong></p><br/>";
		}
		//output form
		echo $depositmess;
	...
	public function validatePayment () {
		...
		if(defined('CURLOPT_SSLVERSION') && ($this->sandbox == 1 || $this->params['tls12'] == 1)) {
			//TLS 1.2
