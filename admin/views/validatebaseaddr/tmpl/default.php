	...
	if (baseaddress.length > 0) {
		geocoder.geocode( { 'address': baseaddress}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				...
				jQuery('#vripanel').append('<p class="vri-valid-geoaddress-par"><i class="fas fa-check-circle"></i> <?php echo addslashes(JText::_('VRICHECKDELADDGMAPOK')); ?></p>');
			} else {
				jQuery(".vrivalidategmapbaseaddrp").addClass("vrierrorbg");
				jQuery('#vripanel').append('<p class="vri-invalid-geoaddress-par"><i class="fas fa-times-circle"></i> <?php echo addslashes(JText::_('VRICHECKDELADDGMAPERR2')); ?>'+status+'</p>');
			}
		});
	} else {
		jQuery(".vrivalidategmapbaseaddrp").addClass("vrierrorbg");
		jQuery('#vripanel').append('<p class="vri-invalid-geoaddress-par"><i class="fas fa-times-circle"></i> <?php echo addslashes(JText::_('VRICHECKDELADDGMAPERR1')); ?></p>');
	}
}
...
<div id="vripanel">
	<p class="vrivalidategmapbaseaddrp"><?php echo JText::_('VRICONFDELBASEADDR'); ?>: <?php echo $pbaseaddress; ?> <span id="vribaseposition"></span></p>
</div>
