...
if (is_array($all_locations)) {
	...
	$loc_options .= '<optgroup label="'.JText::_('VRIORDERSLOCFILTERPICK').'">'."\n";
	...
	$loc_options .= '<optgroup label="'.JText::_('VRIORDERSLOCFILTERDROP').'">'."\n";
	foreach ($all_locations as $location) {
	...
	$loc_options .= '</optgroup>'."\n";
	$loc_options .= '<optgroup label="'.JText::_('VRIORDERSLOCFILTERPICKDROP').'">'."\n";
	foreach ($all_locations as $location) {
...
<form action="index.php?option=com_vikrentitems&task=orders" method="post" name="adminForm" id="adminForm" class="vri-allorders-fm">
	...
	<div id="filter-bar" class="btn-toolbar vri-btn-toolbar" style="width: 100%; display: inline-block;">
		<div class="btn-group pull-left input-append">
			<input type="text" name="filtnc" id="filtnc" autocomplete="off" placeholder="<?php echo JText::_('VRIFILTCNAMECNUMB'); ?>" value="<?php echo (strlen($filtnc) > 0 ? $filtnc : ''); ?>" size="30" />
		...
		<div class="btn-group pull-left">
			<button type="button" class="btn" id="vri-search-tools-btn" onclick="if(jQuery(this).hasClass('btn-primary')){jQuery('#vri-search-tools-cont').hide();jQuery(this).removeClass('btn-primary');}else{jQuery('#vri-search-tools-cont').show();jQuery(this).addClass('btn-primary');}"><?php echo JText::_('JSEARCH_TOOLS'); ?> <span class="caret"></span></button>
		</div>
		<div class="btn-group pull-left">
			<button type="button" class="btn" onclick="jQuery('#filter-bar, #vri-search-tools-cont').find('input, select').val('');document.getElementById('cust_id').value='';document.adminForm.submit();"><?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?></button>
		</div>
		...
		<div id="vri-search-tools-cont" class="js-stools-container-filters clearfix" style="display: none;">
			...
			$datesel .= '<option value="2"'.(!empty($pdatefilt) && $pdatefilt == 2 ? ' selected="selected"' : '').'>'.JText::_('VRIEXPCSVPICK').'</option>';
			$datesel .= '<option value="3"'.(!empty($pdatefilt) && $pdatefilt == 3 ? ' selected="selected"' : '').'>'.JText::_('VRIEXPCSVDROP').'</option>';
			$datesel .= '</select>';
			echo $datesel;
			?>
			</div>
			<div class="btn-group pull-left" id="vri-dates-cont" style="display: <?php echo (!empty($pdatefilt) && (!empty($pdatefiltfrom) || !empty($pdatefiltto)) ? 'inline-block' : 'none'); ?>;">
				<input type="text" id="vri-date-from" placeholder="<?php echo JText::_('VRNEWSEASONONE'); ?>" value="<?php echo $pdatefiltfrom; ?>" size="10" name="datefiltfrom" />&nbsp;-&nbsp;<input type="text" id="vri-date-to" placeholder="<?php echo JText::_('VRNEWSEASONTWO'); ?>" value="<?php echo $pdatefiltto; ?>" size="10" name="datefiltto" />
			</div>
			<div class="btn-group pull-left">
	...
	<div class="table-responsive">
		<table cellpadding="4" cellspacing="0" border="0" width="100%" class="table table-striped vri-orderslist-table">
			<thead>
				<tr>
					...
					<th class="title left" width="200"><?php echo JText::_( 'VRPVIEWORDERSTOTITEMS' ); ?></th>
					<th class="title left" width="110">
		...
		for ($i = 0, $n = count($rows); $i < $n; $i++) {
			...
			if ($row['status'] == 'confirmed') {
				$status_lbl = "<span class=\"label label-success vri-status-label\">".JText::_('VRCONFIRMED')."</span>";
			} elseif ($row['status'] == 'standby') {
				$status_lbl = "<span class=\"label label-warning vri-status-label\">".JText::_('VRSTANDBY')."</span>";
			} elseif ($row['status'] == 'cancelled') {
				$status_lbl = "<span class=\"label label-error vri-status-label\" style=\"background-color: #d9534f;\">".JText::_('VRCANCELLED')."</span>";
			}
			...
			<tr class="row<?php echo $kk; ?>">
				...
				<td>
					<span<?php echo JText::_('VRIDBTEXTITEMCLOSED') == $row['custdata'] ? ' class="label vri-stopsales-sp vriordersitemclosed" title="'.JText::_('VRIDBTEXTITEMCLOSED').'"' : ''; ?>><?php echo implode(', ', $items_booked_arr); ?></span>
				</td>
				<td>
				...
				<td class="center">
					<?php echo ($row['hourly'] == 1 && $hoursdiff > 0 ? $hoursdiff.' '.JText::_('VRIHOURS') : $row['days']); ?>
				</td>
				<td class="center">
...
jQuery(document).ready(function() {
	// select2 filters
	jQuery('select#iditem-selfilt').select2({width: '150px', placeholder: '<?php echo addslashes(JText::_('VRITEMFILTER')); ?>', allowClear: true});
	jQuery('select#idpayment-selfilt').select2({width: '150px', placeholder: '<?php echo addslashes(JText::_('VRFILTERBYPAYMENT')); ?>', allowClear: true});
	jQuery('select#status-selfilt').select2({width: '150px', placeholder: '<?php echo addslashes(JText::_('VRFILTERBYSTATUS')); ?>', allowClear: true});
	jQuery('select#locfilter').select2({width: '150px', placeholder: '<?php echo addslashes(JText::_('VRIORDERSLOCFILTERANY')); ?>', allowClear: true});
	jQuery('select#datefilt-selfilt').select2({width: '150px', placeholder: '<?php echo addslashes(JText::_('VRFILTERBYDATES')); ?>', allowClear: true});
	//
