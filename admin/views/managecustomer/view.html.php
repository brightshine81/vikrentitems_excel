...
class VikRentItemsViewManagecustomer extends JViewVikRentItems {
	...
	function display($tpl = null) {
		...
		$dbo = JFactory::getDBO();
		if (!empty($cid[0])) {
			$q = "SELECT * FROM `#__vikrentitems_customers` WHERE `id`=".(int)$idcust.";";
			$dbo->setQuery($q);
			$dbo->execute();
			if ($dbo->getNumRows() == 1) {
	...
	protected function addToolBar() {
		...
		if (!empty($cid[0])) {
			//edit
			JToolBarHelper::title(JText::_('VRMAINMANAGECUSTOMERTITLE'), 'vikrentitems');
