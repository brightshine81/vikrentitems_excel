...
class VikRentItemsViewCustomers extends JViewVikRentItems {
	...
	function display($tpl = null) {
		...
		$q = "SELECT SQL_CALC_FOUND_ROWS *,(SELECT COUNT(*) FROM `#__vikrentitems_customers_orders` WHERE `#__vikrentitems_customers_orders`.`idcustomer`=`#__vikrentitems_customers`.`id`) AS `tot_bookings`,(SELECT `country_name` FROM `#__vikrentitems_countries` WHERE `#__vikrentitems_countries`.`country_3_code`=`#__vikrentitems_customers`.`country`) AS `country_full_name` FROM `#__vikrentitems_customers`".$whereclause." ORDER BY `".$orderby."` ".$ordersort;
	...
	protected function addToolBar() {
		JToolBarHelper::title(JText::_('VRMAINCUSTOMERSTITLE'), 'vikrentitems');
		...
		if (JFactory::getUser()->authorise('core.vri.management', 'com_vikrentitems')) {
			JToolBarHelper::custom( 'exportcustomers', 'file-2', 'file-2', JText::_('VRICSVEXPCUSTOMERS'), false);
			JToolBarHelper::spacer();
		}
		if (JFactory::getUser()->authorise('core.delete', 'com_vikrentitems')) {
			JToolBarHelper::deleteList(JText::_('VRIDELCONFIRM'), 'removecustomers', JText::_('VRMAINCUSTOMERDEL'));
			JToolBarHelper::spacer();
		}
	}
