...
$fprice = "<div class=\"vri-fares-tabs\"><div class=\"dailypricesactive\">".JText::_('VRIDAILYFARES')."</div><div class=\"hourscharges\"><a href=\"index.php?option=com_vikrentitems&task=hourscharges&cid[]=".$iditem."\">".JText::_('VRIHOURSCHARGES')."</a></div><div class=\"hourlyprices\"><a href=\"index.php?option=com_vikrentitems&task=tariffshours&cid[]=".$iditem."\">".JText::_('VRIHOURLYFARES')."</a></div></div>\n";
if (empty($prices)) {
	...
	$colsp = "2";
	$fprice .= "<form name=\"newd\" class=\"vri-fares-frm\" method=\"post\" action=\"index.php?option=com_vikrentitems\" onsubmit=\"javascript: if (!document.newd.ddaysfrom.value.match(/\S/)){alert('".JText::_('VRMSGTWO')."'); return false;} else {return true;}\">\n<div class=\"vri-insertrates-cont\"><span class=\"vri-ratestable-lbl\">".JText::_('VRDAYS').": </span><table><tr><td><span class=\"vri-fares-from-lbl\">".JText::_('VRDAYSFROM')."</span> <input type=\"number\" name=\"ddaysfrom\" id=\"ddaysfrom\" value=\"".(!is_array($prices) ? '1' : '')."\" min=\"1\" /></td><td>&nbsp;&nbsp;&nbsp; <span class=\"vri-fares-to-lbl\">".JText::_('VRDAYSTO')."</span> <input type=\"number\" name=\"ddaysto\" id=\"ddaysto\" value=\"".(!is_array($prices) ? '30' : '')."\" min=\"1\" max=\"999\" /></td></tr></table>\n";
	$fprice .= "<span class=\"vri-ratestable-lbl\">".JText::_('VRDAILYPRICES').": </span><table>\n";
	...
	$fprice .= "<tr><td colspan=\"".$colsp."\" align=\"right\"><input type=\"submit\" class=\"vrisubmitfares btn btn-large btn-success\" name=\"newdispcost\" value=\"".JText::_('VRINSERT')."\"/></td></tr></table></div><input type=\"hidden\" name=\"cid[]\" value=\"".$iditem."\"/><input type=\"hidden\" name=\"task\" value=\"tariffs\"/></form>";
}
$chitemsel = "<select id=\"vri-item-selection\" name=\"cid[]\" onchange=\"javascript: document.vrichitem.submit();\">\n";
foreach ($allc as $cc) {
	$chitemsel .= "<option value=\"".$cc['id']."\"".($cc['id'] == $iditem ? " selected=\"selected\"" : "").">".$cc['name']."</option>\n";
}
$chitemsel .= "</select>\n";
$chitemf = "<form name=\"vrichitem\" method=\"post\" action=\"index.php?option=com_vikrentitems\"><input type=\"hidden\" name=\"task\" value=\"tariffs\"/>".$chitemsel."</form>";
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="table table-striped vri-list-table">
		...
		<tr>
			...
			<th width="20" class="title right" style="text-align: right;">
				<input type="submit" name="modtar" value="<?php echo JText::_( 'VRPVIEWTARTWO' ); ?>" onclick="vrRateSetTask(event);" class="btn" /> &nbsp; <input type="checkbox" onclick="Joomla.checkAll(this)" value="" name="checkall-toggle">
			</th>
		</tr>
		</thead>
	<?php
