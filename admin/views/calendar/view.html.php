...
class VikRentItemsViewCalendar extends JViewVikRentItems {
	...
	function display($tpl = null) {
		...
		if (!empty($ppickupdate) && !empty($preleasedate)) {
			if (VikRentItems::dateIsValid($ppickupdate) && VikRentItems::dateIsValid($preleasedate)) {
				...
				if ($second > $first) {
					...
					$checknunits = intval($psetclosed) == 1 ? 1 : $pitemquant;
					if ($itemrows['units'] >= $pitemquant && VikRentItems::itemBookable($itemrows['id'], $itemrows['units'], $first, $second, $checknunits) && VikRentItems::itemNotLocked($itemrows['id'], $itemrows['units'], $first, $second, $checknunits)) {
						...
						//
						$allnewbusy = array();
						$forend = intval($psetclosed) == 1 ? $itemrows['units'] : $pitemquant;
						...
						//
						$pordstatus = intval($psetclosed) == 1 ? 'confirmed' : $pordstatus;
						$q = "INSERT INTO `#__vikrentitems_orders` (`custdata`,`ts`,`status`,`days`,`ritiro`,`consegna`,`custmail`,`sid`,`idplace`,`idreturnplace`,`idpayment`,`hourly`,`order_total`,`locationvat`,`deliverycost`) VALUES(".$dbo->quote($pcustdata).",'".$actnow."','".$pordstatus."','".$daysdiff."','".$first."','".$second."',".$dbo->quote($pcustmail).",'".$sid."',".(!empty($ppickuploc) ? "'".$ppickuploc."'" : "NULL").",".(!empty($pdropoffloc) ? "'".$pdropoffloc."'" : "NULL").",".$dbo->quote($paymentmeth).",'".($checkhourly ? "1" : "0")."', ".($set_total > 0 ? $dbo->quote($set_total) : "NULL").", ".(strlen($locationvat) > 0 ? "'".$locationvat."'" : "NULL").", ".(!empty($deliverycost) ? (float)$deliverycost : "NULL").");";
						$dbo->setQuery($q);
						$dbo->execute();
						$newoid = $dbo->insertid();
						$msg = $newoid;
						...
						$pitemquant = intval($psetclosed) == 1 ? $itemrows['units'] : $pitemquant;
						$q = "INSERT INTO `#__vikrentitems_ordersitems` (`idorder`,`iditem`,`itemquant`,`deliveryaddr`,`deliverydist`,`cust_cost`,`cust_idiva`) VALUES(".(int)$newoid.",".(int)$itemrows['id'].",".(int)$pitemquant.",".(!empty($pdeliveryaddr) ? $dbo->quote($pdeliveryaddr) : "NULL").",".(!empty($pdeliverydist) ? floatval($pdeliverydist) : "NULL").", ".($pcust_cost > 0 ? $dbo->quote($pcust_cost) : "NULL").", ".($pcust_cost > 0 && !empty($ptaxid) ? $dbo->quote($ptaxid) : "NULL").");";
						$dbo->setQuery($q);
						$dbo->execute();
						//Customer Booking
						...
						if ($pordstatus == 'standy') {
							...
							$dbo->setQuery($q);
							$dbo->execute();
							$mainframe->enqueueMessage(JText::_('VRIQUICKRESWARNSTANDBY'));
		...
		$q = "SELECT `b`.*,`ob`.`idorder` FROM `#__vikrentitems_busy` AS `b` LEFT JOIN `#__vikrentitems_ordersbusy` `ob` ON `ob`.`idbusy`=`b`.`id` WHERE `b`.`iditem`='".$itemrows['id']."' AND (`b`.`ritiro`>=".$mints." OR `b`.`consegna`>=".$mints.");";
		$dbo->setQuery($q);
		$dbo->execute();
		if ($dbo->getNumRows() > 0) {
	...
	protected function addToolBar() {
		JToolBarHelper::title(JText::_('VRMAINCALTITLE'), 'vikrentitems');
