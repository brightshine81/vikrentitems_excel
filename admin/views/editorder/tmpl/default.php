...
$row['closure'] = (int)(JText::_('VRIDBTEXTITEMCLOSED') == $row['custdata']);
...
foreach ($items as $koi => $oi) {
	...
	$isdue += VikRentItems::sayCostPlusIva($tar[0]['cost'] * $oi['itemquant'], $tar[0]['idprice'], $row);
...
function changePayment() {
	...
	if (newpayment != '') {
		...
		if (confirm('<?php echo addslashes(JText::_('VRCHANGEPAYCONFIRM')); ?>' + paymentname + '?')) {
	...
	<form name="adminForm" id="adminForm" action="index.php" method="post">	
		...
		?>
			...
			<div class="vri-bookdet-wrap">
				...
				<div class="vri-bookdet-foot">
					<?php echo ($row['hourly'] == 1 && $hours_rented > 0 ? $hours_rented.' '.JText::_('VRIHOURS') : $row['days']); ?>
				</div>
			</div>
			<div class="vri-bookdet-wrap">
				<div class="vri-bookdet-head">
		...
		<div class="vri-bookingdet-innertop">
			...
			if (is_array($busy) || $row['status'] == "standby") {
				...
				<div class="vri-bookingdet-command">
					<button onclick="document.location.href='index.php?option=com_vikrentitems&task=editbusy&cid[]=<?php echo $row['id']; ?>';" class="btn btn-secondary" type="button"><i class="icon-pencil"></i> <?php echo JText::_('VRMODRES'); ?></button>
				</div>
				<?php
			}
			if ((is_array($items) && !empty($items[0]['idtar'])) || $is_cust_cost) {
				?>
				<div class="vri-bookingdet-command">
					<button onclick="window.open('<?php echo JURI::root(); ?>index.php?option=com_vikrentitems&task=vieworder&sid=<?php echo $row['sid']; ?>&ts=<?php echo $row['ts']; ?>', '_blank');" type="button" class="btn btn-secondary"><i class="icon-eye"></i> <?php echo JText::_('VRIFRONTVIEWSTANDBYORDER'); ?></button>
				</div>
				<?php
			}
			if (($row['status'] == "confirmed" || ($row['status'] == "standby" && !empty($row['custmail']))) && ((is_array($items) && !empty($items[0]['idtar'])) || ($is_cust_cost))) {
				?>
				<div class="vri-bookingdet-command">
					<button class="btn btn-success" type="button" onclick="document.location.href='index.php?option=com_vikrentitems&task=resendordemail&cid[]=<?php echo $row['id']; ?>';"><i class="icon-mail"></i> <?php echo JText::_('VRIRESENDORDEMAIL'); ?></button>
				</div>
				<?php
				if ($row['status'] == "confirmed") {
				?>
				<div class="vri-bookingdet-command">
					<button class="btn btn-success" type="button" onclick="document.location.href='index.php?option=com_vikrentitems&task=resendordemail&sendpdf=1&cid[]=<?php echo $row['id']; ?>';"><i class="icon-mail"></i> <?php echo JText::_('VRIRESENDORDEMAILANDPDF'); ?></button>
				</div>
				<?php
				}
			}
			if ($row['status'] == "standby") {
				?>
				<div class="vri-bookingdet-command">
					<button class="btn btn-success" type="button" onclick="if (confirm('<?php echo addslashes(JText::_('VRSETORDCONFIRMED')); ?> ?')) {document.location.href='index.php?option=com_vikrentitems&task=setordconfirmed&cid[]=<?php echo $row['id']; ?>';}"><i class="vriicn-checkmark"></i> <?php echo JText::_('VRSETORDCONFIRMED'); ?></button>
				</div>
				<?php
			}
			if ($row['status'] != 'confirmed' || $row['closure'] > 0) {
				?>
				<div class="vri-bookingdet-command">
					<button type="button" class="btn btn-danger" onclick="if (confirm('<?php echo addslashes(JText::_('VRIDELCONFIRM')); ?>')){document.location.href='index.php?option=com_vikrentitems&task=removeorders&cid[]=<?php echo $row['id']; ?>';}"><i class="vriicn-bin"></i> <?php echo JText::_('VRMAINEBUSYDEL'); ?></button>
				</div>
				<?php
			}
			?>
			...
			<div class="vri-bookingdet-tabs">
				<div class="vri-bookingdet-tab vri-bookingdet-tab-active" data-vritab="vri-tab-details"><?php echo JText::_('VRIBOOKDETTABDETAILS'); ?></div>
				<div class="vri-bookingdet-tab" data-vritab="vri-tab-admin"><?php echo JText::_('VRIBOOKDETTABADMIN'); ?></div>
			</div>
		</div>
			...
			<div class="vri-bookingdet-innercontainer">
				...
				if ($contracted) {
					...
					if ($row['status'] == "confirmed") {
						?>
						<div>
							<span class="label label-success"><?php echo JText::_('VRICONFIRMATIONNUMBER'); ?> <span class="badge"><?php echo $row['sid'].'_'.$row['ts']; ?></span></span>
						</div>
						<?php
					}
					?>
						<div>
							<span class="label label-success"><span class="badge"><a href="<?php echo VRI_SITE_URI; ?>resources/pdfs/<?php echo $row['id'].'_'.$row['ts']; ?>.pdf" target="_blank"><i class="vriicn-file-text2"></i><?php echo JText::_('VRIDOWNLOADPDF'); ?></a></span></span>
						</div>
<div>
                            <span class="label label-success"><span class="badge"><a href="index.php?option=com_vikrentitems&task=recreatepdf&cid[]=<?=$row['id']?>"><i class="vriicn-file-text2"></i><?php echo JText::_('Recreate PDF'); ?></a></span></span>
                        </div>
<div class="recreate_date">
                            <span class="label label-success">Last Update: <?=$row['recreate_date']?></span>
                        </div>
					</div>
					<?php
				}
				...
				?>
					<div class="vri-bookingdet-detcont vri-hidein-print">
						<label for="custmail"><?php echo JText::_('VRQRCUSTMAIL'); ?></label>
						<input type="text" name="custmail" id="custmail" value="<?php echo $row['custmail']; ?>" size="25"/>
						<?php if (!empty($row['custmail'])) : ?> <button type="button" class="btn btn-secondary" onclick="vriToggleSendEmail();" style="vertical-align: top;"><i class="vriicn-envelop"></i><?php echo JText::_('VRSENDEMAILACTION'); ?></button><?php endif; ?>
					</div>
				<?php
				}
				?>
					...
					<div class="table-responsive">
						...
						foreach ($items as $koi => $oi) {
							...
							<tr class="vri-bookingdet-summary-item">
								<td class="vri-bookingdet-summary-item-firstcell">
									<div class="vri-bookingdet-summary-itemnum"><i class="vriicn-stack"></i> <?php echo JText::_('VREDITORDERTHREE').' '.$num; ?></div>
									<?php
								...
								<td>
									...
									if ((!empty($oi['cust_cost']) && $oi['cust_cost'] > 0.00)) {
										echo JText::_('VRIRENTCUSTRATEPLAN');
							...
							<tr class="vri-bookingdet-summary-options">
								<td class="vri-bookingdet-summary-options-title"><?php echo $counter == 1 ? JText::_('VREDITORDEREIGHT') : '&nbsp;'; ?></td>
								<td>
							...
							<tr class="vri-bookingdet-summary-custcosts">
								<td class="vri-bookingdet-summary-custcosts-title"><?php echo JText::_('VRIMAILDELIVERYTO'); ?></td>
								<td>
							...
							if (!empty($oi['extracosts'])) {
								...
								foreach ($cur_extra_costs as $eck => $ecv) {
									...
									?>
							<tr class="vri-bookingdet-summary-custcosts">
								<td class="vri-bookingdet-summary-custcosts-title"><?php echo $counter == 1 ? JText::_('VRPEDITBUSYEXTRACOSTS') : '&nbsp;'; ?></td>
								<td>
						...
						if (!empty($row['idplace']) && !empty($row['idreturnplace'])) {
							...
							<tr class="vri-bookingdet-summary-custcosts">
								...
								<td>
									<span class="vri-bookingdet-summary-lbl"><?php echo JText::_('VREDITORDERTEN'); ?></span>
								</td>
								<td>
						...
						if (!empty($totdelivery) && $totdelivery > 0) {
							...
							<tr class="vri-bookingdet-summary-custcosts">
								<td class="vri-bookingdet-summary-custcosts-title">&nbsp;</td>
								<td>
									<span class="vri-bookingdet-summary-lbl"><?php echo JText::_('VRIMAILTOTDELIVERY'); ?></span>
								</td>
								<td>
						...
						if (strlen($row['coupon']) > 0) {
							...
							<tr class="vri-bookingdet-summary-coupon">
								<td><?php echo JText::_('VRICOUPON'); ?></td>
								<td>
							...
							<tr class="vri-bookingdet-summary-total">
								<td>
									<span class="vrapplydiscsp" onclick="toggleDiscount(this);">
										<i class="fa fa-chevron-down" title="<?php echo JText::_('VRIAPPLYDISCOUNT'); ?>"></i>
									</span>
								</td>
								<td>
									<span class="vri-bookingdet-summary-lbl"><?php echo JText::_('VREDITORDERNINE'); ?></span>

									<div class="vridiscenter" id="vridiscenter" style="display: none;">
										<div class="vridiscenter-entry">
											<span class="vridiscenter-label"><?php echo JText::_('VRIAPPLYDISCOUNT'); ?>:</span><span class="vridiscenter-value"><?php echo $currencyname; ?> <input type="number" step="any" name="admindisc" value="" size="4"/></span>
										</div>
										<div class="vridiscenter-entrycentered">
											<button type="submit" class="btn btn-success"><?php echo JText::_('VRIAPPLYDISCOUNTSAVE'); ?></button>
										</div>
									</div>
								</td>
								<td>
						...
						if (!empty($row['totpaid']) && $row['totpaid'] > 0) {
							...
							<tr class="vri-bookingdet-summary-totpaid">
								<td>&nbsp;</td>
								<td><?php echo JText::_('VRIAMOUNTPAID'); ?></td>
							...
							<tr class="vri-bookingdet-summary-totpaid vri-bookingdet-summary-totremaining">
								<td>&nbsp;</td>
								<td>
									<div><?php echo JText::_('VRITOTALREMAINING'); ?></div>
								</td>
			...
			<div class="vri-bookingdet-innercontainer">
				<div class="vri-bookingdet-admindata">
					...
					if (is_array($payments)) {
						$chpayment = '<div><select name="newpayment" id="newpayment" onchange="changePayment();"><option value="">'.JText::_('VRICHANGEPAYLABEL').'</option>';
				...
				?>
					<div class="vri-bookingdet-admin-entry">
						<label for="newlang"><?php echo JText::_('VRIBOOKINGLANG'); ?></label>
				...
				<div class="vri-bookingdet-noteslogs">
					<div class="vri-bookingdet-noteslogs-btns">
						<div class="vri-bookingdet-noteslogs-btn vri-bookingdet-noteslogs-btn-active">
							<a href="javascript: void(0);" onclick="javascript: vriToggleNotes(this);"><?php echo JText::_('VRIADMINNOTESTOGGLE'); ?></a>
						</div>
					<?php
					if (!empty($row['paymentlog'])) {
						?>
						<div class="vri-bookingdet-noteslogs-btn">
							<a href="javascript: void(0);" id="vri-trig-paylogs" onclick="javascript: vriToggleLog(this);"><?php echo JText::_('VRIPAYMENTLOGTOGGLE'); ?></a>
					...
					<div class="vri-bookingdet-noteslogs-cont">
						<div id="vriadminnotesdiv" style="display: block;">
							...
							<br clear="all"/>
							<input type="submit" name="updadmnotes" value="<?php echo JText::_('VRIADMINNOTESUPD'); ?>" class="btn btn-secondary" />
						</div>
					<?php
					if (!empty($row['paymentlog'])) {
						?>
		...
		<div id="vri-overlay-email-cont" style="display: none;">
			...
			?>
				...
				<div class="vri-calendar-cfields-bottom">
					<button type="submit" class="btn"><i class="vriicn-envelop"></i><?php echo JText::_('VRSENDEMAILACTION'); ?></button>
				</div>
...
function vriLoadEmailTpl(tplind) {
	...
	if (tplind.substr(0, 2) == 'rm') {
		if (confirm('<?php echo addslashes(JText::_('VRIDELCONFIRM')); ?>')) {
